...
export function progressJobs(progress, installProgress, readyPods, pods, deployments, events) {
...
export function canContinueDeployStep(stepID, form, selectedCredential, analysis) {
...
export function monorepoComponentsFromAnalysis(applicationName, components = []) {
...
export function deployFormFromApplicationSpec(current, repoFullName, branch, data) {
  const doc = data.document || {};
  const spec = doc.spec || {};
...
export function monorepoComponentHost(component, form, selectedCloudflareDomain) {
...
export function buildMonorepoApplicationPayload(form, githubCredentialID, credentials) {
  const selectedCF = (credentials.domains || []).find((domain) => String(domain.credential_id) === String(form.cloudflare_credential_id) && String(domain.zone_id) === String(form.cloudflare_zone_id))
    || credentials.cloudflare.find((cred) => String(cred.id) === String(form.cloudflare_credential_id));
  return {
    name: form.name,
