...
export default function DeploymentsView({
  ...
  const processRows = (processes || [])
    .filter((process) => process.type === "deployment")
    .map((process) => {
      const project =
        process.project ||
        (projects || []).find((row) => row.id === process.project_id) ||
        {};
      ...
      return {
        ...
        status,
        duration: process.updated_at
          ? shortRelativeDuration(process.updated_at)
          : "-",
        repo:
          project.github_repo ||
          imageRepoName(deployment.image_ref || project.image_reference) ||
  ...
  const fallbackRows = processRows.length
    ? []
    : (runtimeDeployments || []).map((deployment, index) => ({
      ...
      <div className="deployment-list">
        {rows.map((row) => (
          <Button
            ...
            key={row.id}
            onClick={() => row.process && onOpenProcess?.(row.process)}
          >
            ...
            <span className="deploy-branch">
              <span>
                <GitBranch size={18} /> <b>{row.branch}</b>
              </span>
