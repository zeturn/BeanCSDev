...
func (h *RuntimeHandler) Register(r fiber.Router) {
	...
	r.Get("/runtime/nodes/:name/health", middleware.RequireAPIScope(service.ScopeRuntimeRead), h.nodeHealth)
	r.Patch("/runtime/nodes/:name/labels", middleware.RequireAPIScope(service.ScopeRuntimeWrite), h.patchNodeLabels)
	r.Put("/runtime/nodes/:name/taints", middleware.RequireAPIScope(service.ScopeRuntimeWrite), h.updateNodeTaints)
	r.Post("/runtime/nodes/:name/cordon", middleware.RequireAPIScope(service.ScopeRuntimeWrite), h.cordonNode)
	r.Post("/runtime/nodes/:name/uncordon", middleware.RequireAPIScope(service.ScopeRuntimeWrite), h.uncordonNode)
	r.Post("/runtime/nodes/:name/drain", middleware.RequireAPIScope(service.ScopeRuntimeWrite), h.drainNode)
	r.Delete("/runtime/nodes/:name", middleware.RequireAPIScope(service.ScopeRuntimeWrite), h.deleteNode)
	r.Get("/runtime/namespaces", middleware.RequireAPIScope(service.ScopeRuntimeRead), h.namespaces)
	r.Post("/runtime/namespaces", middleware.RequireAPIScope(service.ScopeRuntimeWrite), h.createNamespace)
	r.Get("/runtime/namespaces/:name", middleware.RequireAPIScope(service.ScopeRuntimeRead), h.namespaceDetail)
	r.Patch("/runtime/namespaces/:name", middleware.RequireAPIScope(service.ScopeRuntimeWrite), h.patchNamespace)
	r.Put("/runtime/namespaces/:name/resource-quotas", middleware.RequireAPIScope(service.ScopeRuntimeWrite), h.upsertResourceQuota)
	r.Delete("/runtime/namespaces/:name/resource-quotas/:quota", middleware.RequireAPIScope(service.ScopeRuntimeWrite), h.deleteResourceQuota)
	r.Put("/runtime/namespaces/:name/limit-ranges", middleware.RequireAPIScope(service.ScopeRuntimeWrite), h.upsertLimitRange)
	r.Delete("/runtime/namespaces/:name/limit-ranges/:limit", middleware.RequireAPIScope(service.ScopeRuntimeWrite), h.deleteLimitRange)
	r.Put("/runtime/namespaces/:name/permissions", middleware.RequireAPIScope(service.ScopeRuntimeWrite), h.upsertNamespacePermission)
	r.Delete("/runtime/namespaces/:name/permissions/:permission", middleware.RequireAPIScope(service.ScopeRuntimeWrite), h.deleteNamespacePermission)
	r.Put("/runtime/namespaces/:name/isolation", middleware.RequireAPIScope(service.ScopeRuntimeWrite), h.setNamespaceIsolation)
	r.Delete("/runtime/namespaces/:name", middleware.RequireAPIScope(service.ScopeRuntimeWrite), h.deleteNamespace)
	r.Get("/runtime/pods/:namespace/:name/logs", middleware.RequireAPIScope(service.ScopeRuntimeRead), h.podLogs)
	r.Delete("/runtime/pods/:namespace/:name", middleware.RequireAPIScope(service.ScopeRuntimeWrite), h.deletePod)
	r.Post("/runtime/services", middleware.RequireAPIScope(service.ScopeRuntimeWrite), h.createService)
	r.Put("/runtime/services/:namespace/:name", middleware.RequireAPIScope(service.ScopeRuntimeWrite), h.updateService)
	r.Delete("/runtime/services/:namespace/:name", middleware.RequireAPIScope(service.ScopeRuntimeWrite), h.deleteService)
	r.Post("/runtime/ingresses", middleware.RequireAPIScope(service.ScopeRuntimeWrite), h.createIngress)
	r.Put("/runtime/ingresses/:namespace/:name", middleware.RequireAPIScope(service.ScopeRuntimeWrite), h.updateIngress)
	r.Delete("/runtime/ingresses/:namespace/:name", middleware.RequireAPIScope(service.ScopeRuntimeWrite), h.deleteIngress)
	r.Post("/runtime/network-policies", middleware.RequireAPIScope(service.ScopeRuntimeWrite), h.createNetworkPolicy)
	r.Put("/runtime/network-policies/:namespace/:name", middleware.RequireAPIScope(service.ScopeRuntimeWrite), h.updateNetworkPolicy)
	r.Delete("/runtime/network-policies/:namespace/:name", middleware.RequireAPIScope(service.ScopeRuntimeWrite), h.deleteNetworkPolicy)
	r.Get("/projects/:id/status", middleware.RequireAPIScope(service.ScopeRuntimeRead), middleware.ProjectAccess(h.db), h.status)
	r.Get("/projects/:id/logs", middleware.RequireAPIScope(service.ScopeRuntimeRead), middleware.ProjectAccess(h.db), h.logs)
	r.Post("/projects/:id/restart", middleware.RequireAPIScope(service.ScopeProjectsDeploy), middleware.ProjectOwner(h.db), h.restart)
	r.Post("/projects/:id/scale", middleware.RequireAPIScope(service.ScopeRuntimeWrite), middleware.ProjectOwner(h.db), h.scale)
}
